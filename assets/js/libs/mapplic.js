...
;(function($) {
	...
	var Mapplic = function(element) {
		...
		self.loc = {
			more: 'More',
			search: 'Search',
			iconfile: 'assets/images/svg/icons.svg'
		}
		...
		function Tooltip() {
			...
			this.show = function(location) {
				if (location) {
					...
					if (location.stores) {
						var popupStoresContent = "";

						location.stores.map(function(e) {
							popupStoresContent += "<a class='storeLine' href='#'>";
							popupStoresContent += "<div class='storeImg'><img src='"+e.img+"' alt='"+e.title+"' /></div><div class='storeDesc'>";
							popupStoresContent += "<h4 class='storeTitle'>"+e.title+"</h4>";
							popupStoresContent += "<span class='storeOpen'>"+e.open+"</span><br>";
							popupStoresContent += "<span class='storeMenu'>MENU</span>";
							popupStoresContent += "</div></a>";
						});
						this.desc.html(popupStoresContent);
					} else if (location.description) {
						this.desc.html("<div class='storeLine'><div>"+location.description+"</div></div>");
					}else {
						this.desc.empty();
					}
		...
		self.addLocations = function(locations, levelid) {
			$.each(locations, function(index, location) {
				...
				if (location.pin.indexOf('hidden') == -1) {
					...
					if (location.stores) {
						location.storesLength = location.stores.length.toString();
						pin.attr('data-stores', location.storesLength);
					}
